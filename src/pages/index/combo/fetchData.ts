...
export async function fetchMadeAts(selectedLicense: string) {
  ...
  const { data, error } = await supabase.rpc("get_distinct_made_at", {
    license_name: selectedLicense,
  }); // RPC 함수 호출
...
export async function fetchTests() {
  const { data, error } = await supabase.rpc("fetch_tests_with_license"); // RPC 함수 호출
