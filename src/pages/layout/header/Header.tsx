    ...
    <div className='flex justify-between items-center p-4 border-b h-[60px]'>
      ...
      <div className='text-xl font-bold text-gray-900 xs:tracking-tight xs:text-base xs:font-semibold'>
        {isTest ? (
          ...
          <button onClick={goBack}>미용필기시험</button>
        )}
      </div>

      {!isTest && (
        <div className='text-right'>
          {/* 헤더는 보조 액션 — 터콰이즈는 본문 CTA 하나에만 */}
          <Button variant='default' onClick={goAlmond} className='w-20 mr-2'>
            무료문제집
          </Button>
          {user ? (
            <>
            ...
            <>
              <Button variant='outline' onClick={handleSignIn} className='w-28'>
                로그인/회원가입
              </Button>
            </>
          )}
